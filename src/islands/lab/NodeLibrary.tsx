...
export default function NodeLibrary({ onAddNode }: NodeLibraryProps) {
  ...
  return (
    <div className="w-64 bg-hw-panel border-r-2 border-hw-border shadow-md flex flex-col h-full shrink-0 z-20 relative min-h-0 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar p-3 flex flex-col gap-2 min-h-0 h-0">
        {nodeTypes.map(def => (
          <button
            key={def.type}
